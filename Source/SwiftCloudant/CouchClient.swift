...
public extension CouchDBClient {
    ...
    /**
     Creates a CouchDBClient instance using credientials from the Cloud Foundry environment variable.
     ...
     - parameter configuration: configuration options for the client.
     */
    convenience init(vcapServices: String, instanceName: String?, configuration: ClientConfiguration = ClientConfiguration(shouldBackOff: false)) throws {
        var cloudant: [String:Any]?

        if let data = vcapServices.data(using: .utf8), let app = try? JSONSerialization.jsonObject(with: data, options: []) as? [String: Any] {
            if let services = app?["cloudantNoSQLDB"] as? [[String: Any]] {
                if instanceName == nil {
                    if services.count == 1 {
                        cloudant = services.first!
                    } else {
        ...
        if let credentials = cloudant?["credentials"] as? [String: Any], let urlStr = credentials["url"] as? String {
            if let url = URL(string: urlStr) {
                self.init(url: url, username: url.user, password: url.password, configuration: configuration)
            } else {
                throw Error.invalidVCAP // failed to parse URL
            }
        } else {
            throw Error.missingCloudantService
        }
